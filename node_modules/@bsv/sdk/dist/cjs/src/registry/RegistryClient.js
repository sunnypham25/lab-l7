...
class RegistryClient {
    constructor(wallet = new index_js_1.WalletClient(), options = {}, originator) {
        this.wallet = wallet;
        this.originator = originator;
        this.acceptDelayedBroadcast = options.acceptDelayedBroadcast ?? false;
        this.resolver = options.resolver ?? new index_js_4.LookupResolver();
    }
    /**
    ...
    async registerDefinition(data) {
        ...
        const pushdrop = new index_js_5.PushDrop(this.wallet, this.originator);
        ...
        const result = await broadcaster.broadcast(index_js_3.Transaction.fromAtomicBEEF(tx));
        return result;
    }
    /**
    ...
    async listOwnRegistryEntries(definitionType) {
        ...
        for (const output of outputs) {
            ...
            try {
                ...
                const tx = index_js_3.Transaction.fromBEEF(BEEF, txid);
    ...
    async removeDefinition(registryRecord) {
        ...
        const partialTx = index_js_3.Transaction.fromAtomicBEEF(signableTransaction.tx);
        ...
        const result = await broadcaster.broadcast(index_js_3.Transaction.fromAtomicBEEF(signedTx));
        return result;
    }
    /**
