...
const VerifiableCertificate_js_1 = require("../auth/certificates/VerifiableCertificate.js");
...
class IdentityClient {
    constructor(wallet, options = index_js_1.DEFAULT_IDENTITY_CLIENT_OPTIONS, originator) {
        ...
        this.originator = originator;
        this.wallet = wallet ?? new index_js_2.WalletClient('auto', this.originator);
    ...
    async resolveByAttributes(args, overrideWithContacts = true) {
        ...
        const contactsPromise = overrideWithContacts
            ? this.contactsManager.getContacts().catch(() => [])
            : Promise.resolve([]);
        try {
     ...
     */
    async resolveViaLookup(query) {
        const resolver = new index_js_6.LookupResolver({ networkPreset: 'mainnet' });
        ...
        const certificates = [];
        const anyoneWallet = new ProtoWallet_js_1.default('anyone');
        for (const output of lookupResult.outputs) {
            try {
                const tx = index_js_3.Transaction.fromBEEF(output.beef);
                const decodedOutput = index_js_4.PushDrop.decode(tx.outputs[output.outputIndex].lockingScript);
                const certData = JSON.parse(index_js_5.Utils.toUTF8(decodedOutput.fields[0]));
                const verifiableCert = new VerifiableCertificate_js_1.VerifiableCertificate(certData.type, certData.serialNumber, certData.subject, certData.certifier, certData.revocationOutpoint, certData.fields, certData.keyring, certData.signature);
